...
function Seat1({ idx, id, selectedType, hesaplaLeft, hesaplaTop, handleClick, selected, clickList }: PropValues) {
  ...
  return (
    <div className={`selectedType${selectedType} koltuk${idx + 1}  ${clickList?.length > 0 && clickList[idx]?.clicked === true ? "selected" : ""}`} onClick={() => {
      console.log("clicked", idx); handleClick(idx)
    }} style={{ left: hesaplaLeft(idx), top: hesaplaTop(idx) }} id={idx} title="">{id + 1}</div>
  )
}
